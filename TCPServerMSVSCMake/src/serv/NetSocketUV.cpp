...
{	
	...
	if (udp_tcp)
	{
		...
		if (listen)
		{
			...
			assert(b == 0);
			uv_tcp_t* rtsp = GetPtrTCP(sock);
			sockaddr_in* rtsp_addr = new sockaddr_in;
			i = uv_ip4_addr("0.0.0.0", SERVER_RTSP_PORT, rtsp_addr);
			assert(i == 0);
			b = uv_tcp_bind(rtsp, (sockaddr*)rtsp_addr, 0);
			assert(b == 0);
