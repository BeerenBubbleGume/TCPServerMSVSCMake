...
void OnAccept(uv_stream_t* stream, int status)
{
	...
	if (uv_accept(stream, (uv_stream_t*)client) == 0) // делаетс€ попытка установить соединение
	{
	...
	else
	{
		...
		uv_close((uv_handle_t*)client, OnCloseSocket); // ”дал€ем сокет, который был создан дл€ соединени€
	}
}
